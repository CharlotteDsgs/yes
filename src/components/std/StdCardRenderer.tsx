...
type ElStyles = Record<string, { font?: string; color?: string; size?: number; uppercase?: boolean | "capitalize"; dx?: number; dy?: number; hidden?: boolean }>;
...
function TemplatePhotomaton({ W, H, p, user, label, namesText, dateText, photoUrls, elementStyles }: {
  W: number; H: number; p: Palette; user: UserData;
...
function TemplateOliviers({ W, H, p, user, isStd, namesText, dateText, locationText, elementStyles }: {
  W: number; H: number; p: Palette; user: UserData; isStd: boolean;
  namesText?: string; dateText?: string; locationText?: string; elementStyles?: ElStyles;
}) {
  const rawNames = namesText || `${user.p1 || "Jennifer"} & ${user.p2 || "Nikos"}`;
  const parts = rawNames.split(/\s*[&]\s*/);
  const name1Raw = (parts[0] ?? rawNames).trim(); const name2Raw = (parts[1] ?? "").trim();
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} width={W} height={H} style={{ display: "block" }}>
      ...
      <g transform={`translate(${getDX("names")*W} ${getDY("names")*H})`} style={{ display: elementStyles?.["names"]?.hidden ? "none" : undefined }}>
        <text x={W/2} y={isStd ? H*0.38 : H*0.36} textAnchor="middle" fontFamily={elementStyles?.names?.font ?? "var(--font-script)"} fontSize={getSize("names", W * 0.105)} fill={getColor("names", p.textPrimary)}>{name1Raw}</text>
        ...
        {name2Raw && <text x={W/2} y={isStd ? H*0.57 : H*0.55} textAnchor="middle" fontFamily={elementStyles?.names?.font ?? "var(--font-script)"} fontSize={getSize("names", W * 0.105)} fill={getColor("names", p.textPrimary)}>{name2Raw}</text>}
      </g>
      <g transform={`translate(${getDX("date")*W} ${getDY("date")*H})`} style={{ display: elementStyles?.["date"]?.hidden ? "none" : undefined }}>
        <text x={W/2} y={isStd ? H*0.79 : H*0.755} textAnchor="middle" fontFamily={elementStyles?.date?.font ?? "var(--font-montserrat)"} fontSize={getSize("date", W * 0.028)} fill={getColor("date", p.textPrimary)} letterSpacing="2" style={{ textTransform:"uppercase" as const }}>{displayDate}</text>
      </g>
      {displayLocation && (
        <g transform={`translate(${getDX("location")*W} ${getDY("location")*H})`} style={{ display: elementStyles?.["location"]?.hidden ? "none" : undefined }}>
          <text x={W/2} y={isStd ? H*0.86 : H*0.82} textAnchor="middle" fontFamily={elementStyles?.location?.font ?? "var(--font-serif)"} fontSize={getSize("location", W * 0.030)} fill={getColor("location", p.textSecondary)} fontStyle="italic">{displayLocation}</text>
        </g>
      )}
    </svg>
  );
...
function TemplateRayures({ W, H, p, user, isStd, customPaperBg, label, namesText, dateText, locationText, elementStyles }: {
  ...
  const parts = rawNames.split(/\s*[&]\s*/); const name1Raw = (parts[0] ?? rawNames).trim(); const name2Raw = (parts[1] ?? "").trim();
...
function TemplateLettre({ W, H, paperImage, p, user, fontPreset = "romantique", label, namesText, dateText, locationText, footer, elementStyles, paperFit = "xMidYMid slice" }: {
  W: number; H: number; paperImage: string; p: Palette; user: UserData; fontPreset?: string;
  label?: string; namesText?: string; dateText?: string; locationText?: string; footer?: string;
  elementStyles?: ElStyles; paperFit?: string;
}) {
  const fp = FONT_PRESETS.find(f => f.id === fontPreset) ?? FONT_PRESETS[0];
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} width={W} height={H} style={{ display: "block" }}>
      ...
      <g transform={`translate(${getDX("label")*W} ${getDY("label")*H})`} style={{ display: elementStyles?.["label"]?.hidden ? "none" : undefined }}>
        <text x={W / 2} y={H * 0.22} textAnchor="middle" fontFamily={getFont("label", fp.scriptFont)} fontStyle={fp.scriptItalic ? "italic" : "normal"} fontSize={getSize("label", W * 0.062)} fill={getColor("label", p.textPrimary)} opacity="0.9">{getCase("label", displayLabel)}</text>
      </g>
      <line x1={W * 0.32} y1={H * 0.27} x2={W * 0.68} y2={H * 0.27} stroke={p.accent} strokeWidth="0.7" opacity="0.45"/>
      <g transform={`translate(${getDX("names")*W} ${getDY("names")*H})`} style={{ display: elementStyles?.["names"]?.hidden ? "none" : undefined }}>
        <text x={W / 2} y={H * 0.38} textAnchor="middle" fontFamily={getFont("names", fp.scriptFont)} fontStyle={fp.scriptItalic ? "italic" : "normal"} fontSize={getSize("names", W * 0.072)} fill={getColor("names", p.textPrimary)}>{getCase("names", displayNames)}</text>
      </g>
      <line x1={W * 0.32} y1={H * 0.43} x2={W * 0.68} y2={H * 0.43} stroke={p.accent} strokeWidth="0.7" opacity="0.45"/>
      <g transform={`translate(${getDX("date")*W} ${getDY("date")*H})`} style={{ display: elementStyles?.["date"]?.hidden ? "none" : undefined }}>
        <text x={W / 2} y={H * 0.52} textAnchor="middle" fontFamily={getFont("date", fp.bodyFont)} fontSize={getSize("date", W * 0.030)} fill={getColor("date", p.textPrimary)} letterSpacing="2" fontWeight="500">{getCase("date", displayDate)}</text>
      </g>
      {displayLocation && (
        <g transform={`translate(${getDX("location")*W} ${getDY("location")*H})`} style={{ display: elementStyles?.["location"]?.hidden ? "none" : undefined }}>
          <text x={W / 2} y={H * 0.577} textAnchor="middle" fontFamily={getFont("location", fp.bodyFont)} fontSize={getSize("location", W * 0.027)} fill={getColor("location", p.textSecondary)} letterSpacing="1.5" fontStyle="italic">{getCase("location", displayLocation)}</text>
        </g>
      )}
      <g transform={`translate(${getDX("footer")*W} ${getDY("footer")*H})`} style={{ display: elementStyles?.["footer"]?.hidden ? "none" : undefined }}>
        <text x={W / 2} y={H * 0.835} textAnchor="middle" fontFamily={getFont("footer", fp.scriptFont)} fontStyle={fp.scriptItalic ? "italic" : "normal"} fontSize={getSize("footer", W * 0.042)} fill={getColor("footer", p.textSecondary)} opacity="0.75">{getCase("footer", displayFooter)}</text>
      </g>
    </svg>
  );
}

function TemplateElegant({ W, H, paperImage, p, user, label, namesText, dateText, locationText, footer, elementStyles }: {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} width={W} height={H} style={{ display: "block", outline: "none" }}>
      ...
      <rect x={W * 0.052} y={H * 0.032} width={W * 0.896} height={H * 0.936} fill="none" stroke={p.accent} strokeWidth="0.9" strokeDasharray="2,3.5" opacity="0.7"/>
      <rect x={W * 0.068} y={H * 0.042} width={W * 0.864} height={H * 0.916} fill="none" stroke={p.accent} strokeWidth="0.4" opacity="0.35"/>
      <g transform={`translate(${getDX("label")*W} ${getDY("label")*H})`} style={{ display: elementStyles?.["label"]?.hidden ? "none" : undefined }}>
        <text x={W / 2} y={H * 0.408} textAnchor="middle" fontFamily={elementStyles?.label?.font ?? "var(--font-playfair)"} fontStyle="italic" fontSize={getSize("label", W * 0.046)} fill={getColor("label", "#909090")}>{getCase("label", displayLabel)}</text>
      </g>
      <g transform={`translate(${getDX("names")*W} ${getDY("names")*H})`} style={{ display: elementStyles?.["names"]?.hidden ? "none" : undefined }}>
        <text x={W / 2} y={H * 0.500} textAnchor="middle" fontFamily={elementStyles?.names?.font ?? "'Times New Roman', Georgia, serif"} fontWeight="400" fontSize={getSize("names", W * 0.080)} fill={getColor("names", p.accent)} letterSpacing="3">{nameCase("names", name1Raw)}</text>
        <text x={W / 2} y={H * 0.560} textAnchor="middle" fontFamily="var(--font-script)" fontStyle="italic" fontSize={W * 0.054} fill={getColor("names", p.textSecondary)} opacity="0.75">et</text>
        {name2Raw && <text x={W / 2} y={H * 0.648} textAnchor="middle" fontFamily={elementStyles?.names?.font ?? "'Times New Roman', Georgia, serif"} fontWeight="400" fontSize={getSize("names", W * 0.080)} fill={getColor("names", p.accent)} letterSpacing="3">{nameCase("names", name2Raw)}</text>}
      </g>
      <g transform={`translate(${getDX("date")*W} ${getDY("date")*H})`} style={{ display: elementStyles?.["date"]?.hidden ? "none" : undefined }}>
        <text x={W / 2} y={H * 0.752} textAnchor="middle" fontFamily={elementStyles?.date?.font ?? "var(--font-montserrat)"} fontSize={getSize("date", W * 0.026)} fill={getColor("date", p.textSecondary)} letterSpacing="3.5" style={{ textTransform: "uppercase" as const }}>{getCase("date", displayDate)}</text>
      </g>
      {displayLocation && (
        <g transform={`translate(${getDX("location")*W} ${getDY("location")*H})`} style={{ display: elementStyles?.["location"]?.hidden ? "none" : undefined }}>
          <text x={W / 2} y={H * 0.840} textAnchor="middle" fontFamily={elementStyles?.location?.font ?? "var(--font-serif)"} fontStyle="italic" fontSize={getSize("location", W * 0.030)} fill={getColor("location", p.textPrimary)}>{getCase("location", displayLocation)}</text>
        </g>
      )}
      <g transform={`translate(${getDX("footer")*W} ${getDY("footer")*H})`} style={{ display: elementStyles?.["footer"]?.hidden ? "none" : undefined }}>
        <text x={W / 2} y={H * 0.935} textAnchor="middle" fontFamily={elementStyles?.footer?.font ?? "var(--font-script)"} fontStyle="italic" fontSize={getSize("footer", W * 0.052)} fill={getColor("footer", p.textSecondary)} opacity="0.75">{getCase("footer", displayFooter)}</text>
      </g>
    </svg>
  );
}

function TemplateArbre({ W, H, paperImage, p, user, label, namesText, dateText, locationText, elementStyles }: {
  W: number; H: number; paperImage: string; p: Palette; user: UserData;
  label?: string; namesText?: string; dateText?: string; locationText?: string; elementStyles?: ElStyles;
}) {
  const displayLabel = label ?? "save the date";
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} width={W} height={H} style={{ display: "block", outline: "none" }}>
      <image href={p.paperImage ?? paperImage} x={0} y={0} width={W} height={H} preserveAspectRatio="none"/>
      <g transform={`translate(${getDX("label")*W} ${getDY("label")*H})`} style={{ display: elementStyles?.["label"]?.hidden ? "none" : undefined }}>
        <text x={W / 2} y={H * 0.275} textAnchor="middle" fontFamily={elementStyles?.label?.font ?? "var(--font-montserrat)"} fontSize={getSize("label", W * 0.028)} fill={getColor("label", p.textPrimary)} letterSpacing="5" style={{ textTransform: "uppercase" as const }} opacity="0.9">{getCase("label", displayLabel)}</text>
      </g>
      <g transform={`translate(${getDX("names")*W} ${getDY("names")*H})`} style={{ display: elementStyles?.["names"]?.hidden ? "none" : undefined }}>
        <text x={W / 2} y={H * 0.390} textAnchor="middle" fontFamily={elementStyles?.names?.font ?? "var(--font-script)"} fontStyle="italic" fontSize={getSize("names", W * 0.112)} fill={getColor("names", p.textPrimary)}>{getCase("names", displayNames)}</text>
      </g>
      <text x={W / 2} y={H * 0.497} textAnchor="middle" fontFamily="var(--font-montserrat)" fontSize={W * 0.024} fill={p.textSecondary} letterSpacing="4" style={{ textTransform: "uppercase" as const }} opacity="0.8">vont se marier le</text>
      <g transform={`translate(${getDX("date")*W} ${getDY("date")*H})`} style={{ display: elementStyles?.["date"]?.hidden ? "none" : undefined }}>
        <text x={W / 2} y={H * 0.620} textAnchor="middle" fontFamily={elementStyles?.date?.font ?? "var(--font-script)"} fontStyle="italic" fontSize={getSize("date", W * 0.090)} fill={getColor("date", p.textPrimary)}>{getCase("date", displayDate)}</text>
      </g>
      {displayLocation && (
        <g transform={`translate(${getDX("location")*W} ${getDY("location")*H})`} style={{ display: elementStyles?.["location"]?.hidden ? "none" : undefined }}>
          <text x={W / 2} y={H * 0.752} textAnchor="middle" fontFamily={elementStyles?.location?.font ?? "var(--font-montserrat)"} fontSize={getSize("location", W * 0.025)} fill={getColor("location", p.textSecondary)} letterSpacing="4" style={{ textTransform: "uppercase" as const }} opacity="0.85">{getCase("location", displayLocation)}</text>
        </g>
      )}
    </svg>
  );
...
function TemplateItalySTD({ W, H, paperImage, p, user, label, namesText, dateText, locationText, footer, elementStyles }: {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} width={W} height={H} style={{ display: "block", outline: "none" }}>
      ...
      <g transform={`translate(${getDX("names")*W} ${getDY("names")*H})`} style={{ display: elementStyles?.["names"]?.hidden ? "none" : undefined }}>
        <text x={W / 2} y={H * 0.569} textAnchor="middle" fontFamily={elementStyles?.names?.font ?? "var(--font-serif)"} fontWeight="500" fontSize={getSize("names", W * 0.044)} fill={getColor("names", p.textPrimary)} letterSpacing="4">{getCase("names", displayNames.toUpperCase())}</text>
      </g>
      <g transform={`translate(${getDX("date")*W} ${getDY("date")*H})`} style={{ display: elementStyles?.["date"]?.hidden ? "none" : undefined }}>
        <text x={W / 2} y={H * 0.660} textAnchor="middle" fontFamily={elementStyles?.date?.font ?? "var(--font-serif)"} fontSize={getSize("date", W * 0.034)} fill={getColor("date", p.textPrimary)} letterSpacing="1">{getCase("date", displayDate)}</text>
      </g>
      {displayLocation && (
        <g transform={`translate(${getDX("location")*W} ${getDY("location")*H})`} style={{ display: elementStyles?.["location"]?.hidden ? "none" : undefined }}>
          <text x={W / 2} y={H * 0.735} textAnchor="middle" fontFamily={elementStyles?.location?.font ?? "var(--font-serif)"} fontStyle="italic" fontSize={getSize("location", W * 0.028)} fill={getColor("location", p.textSecondary)} letterSpacing="1">{getCase("location", displayLocation)}</text>
        </g>
      )}
      <g transform={`translate(${getDX("footer")*W} ${getDY("footer")*H})`} style={{ display: elementStyles?.["footer"]?.hidden ? "none" : undefined }}>
        <text x={W / 2} y={H * 0.872} textAnchor="middle" fontFamily={elementStyles?.footer?.font ?? "var(--font-script)"} fontStyle="italic" fontSize={getSize("footer", W * 0.042)} fill={getColor("footer", p.textSecondary)} opacity="0.75">{getCase("footer", displayFooter)}</text>
      </g>
    </svg>
  );
}
...
function TemplateLettreBold({ W, H, paperImage, p, user, namesText, dateText, locationText, footer, elementStyles }: {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} width={W} height={H} style={{ display: "block" }}>
      ...
      <g transform={`translate(${getDX("date")*W} ${getDY("date")*H})`} style={{ display: elementStyles?.["date"]?.hidden ? "none" : undefined }}>
        <text x={W / 2} y={H * 0.438} textAnchor="middle" fontFamily="var(--font-serif)" fontWeight="500" fontSize={getSize("date", W * 0.050)} fill={getColor("date", p.textPrimary)} letterSpacing="3">{getCase("date", displayDate)}</text>
      </g>
      <text x={W / 2} y={H * 0.490} textAnchor="middle" fontFamily="var(--font-serif)" fontStyle="italic" fontSize={W * 0.026} fill={p.textSecondary} opacity="0.7">pour le mariage de</text>
      <line x1={W * 0.30} y1={H * 0.512} x2={W * 0.70} y2={H * 0.512} stroke={p.accent} strokeWidth="0.6" opacity="0.35"/>
      <g transform={`translate(${getDX("names")*W} ${getDY("names")*H})`} style={{ display: elementStyles?.["names"]?.hidden ? "none" : undefined }}>
        <text x={W / 2} y={H * 0.590} textAnchor="middle" fontFamily="var(--font-playfair)" fontWeight="700" fontSize={getSize("names", W * 0.088)} fill={getColor("names", p.textPrimary)} letterSpacing="2">{elementStyles?.["names"]?.uppercase === false ? name1Raw : name1Raw.toUpperCase()}</text>
        ...
        {name2Raw && <text x={W / 2} y={H * 0.720} textAnchor="middle" fontFamily="var(--font-playfair)" fontWeight="700" fontSize={getSize("names", W * 0.088)} fill={getColor("names", p.textPrimary)} letterSpacing="2">{elementStyles?.["names"]?.uppercase === false ? name2Raw : name2Raw.toUpperCase()}</text>}
      </g>
      <line x1={W * 0.30} y1={H * 0.760} x2={W * 0.70} y2={H * 0.760} stroke={p.accent} strokeWidth="0.6" opacity="0.35"/>
      {displayLocation && (
        <g transform={`translate(${getDX("location")*W} ${getDY("location")*H})`} style={{ display: elementStyles?.["location"]?.hidden ? "none" : undefined }}>
          <text x={W / 2} y={H * 0.822} textAnchor="middle" fontFamily="var(--font-montserrat)" fontSize={getSize("location", W * 0.025)} fill={getColor("location", p.textPrimary)} letterSpacing="3.5" style={{ textTransform: "uppercase" as const }}>{getCase("location", displayLocation)}</text>
        </g>
      )}
      <g transform={`translate(${getDX("footer")*W} ${getDY("footer")*H})`} style={{ display: elementStyles?.["footer"]?.hidden ? "none" : undefined }}>
        <text x={W / 2} y={H * 0.892} textAnchor="middle" fontFamily="var(--font-serif)" fontStyle="italic" fontSize={getSize("footer", W * 0.024)} fill={getColor("footer", p.textSecondary)} opacity="0.65">{getCase("footer", displayFooter)}</text>
      </g>
    </svg>
  );
}
...
function TemplateLettrPhoto({ W, H, paperImage, p, user, label, namesText, dateText, photoUrl, elementStyles }: {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} width={W} height={H} style={{ display: "block" }}>
      <image href={paperImage} x={0} y={0} width={W} height={H} preserveAspectRatio="xMidYMid slice"/>
      <g transform={`translate(${getDX("label")*W} ${getDY("label")*H})`} style={{ display: elementStyles?.["label"]?.hidden ? "none" : undefined }}>
        <text x={W / 2} y={H * 0.122} textAnchor="middle" fontFamily={elementStyles?.label?.font ?? "var(--font-script)"} fontStyle="italic" fontSize={getSize("label", W * 0.042)} fill={getColor("label", p.textPrimary)} opacity="0.85">{getCase("label", displayLabel)}</text>
      </g>
      ...
      <g transform={`translate(${getDX("names")*W} ${getDY("names")*H})`} style={{ display: elementStyles?.["names"]?.hidden ? "none" : undefined }}>
        <text x={W / 2} y={fY + fH + H * 0.030} textAnchor="middle" fontFamily={elementStyles?.names?.font ?? "var(--font-montserrat)"} fontSize={getSize("names", W * 0.026)} fill={getColor("names", p.textPrimary)} letterSpacing="3" style={{ textTransform: "uppercase" as const }}>{getCase("names", displayNames)}</text>
      </g>
      <g transform={`translate(${getDX("date")*W} ${getDY("date")*H})`} style={{ display: elementStyles?.["date"]?.hidden ? "none" : undefined }}>
        <text x={W / 2} y={fY + fH + H * 0.082} textAnchor="middle" fontFamily={elementStyles?.date?.font ?? "var(--font-script)"} fontStyle="italic" fontSize={getSize("date", W * 0.040)} fill={getColor("date", p.textPrimary)} opacity="0.82">{getCase("date", displayDate)}</text>
      </g>
    </svg>
  );
}
...
export function TemplateRender({ id, W, H, palette, user, isStd, photoUrl, photoUrls, fontPreset, label, namesText, dateText, locationText, footer, elementStyles, customPaperBg }: {
  ...
  if (id === "photomaton") {
    ...
    return <TemplatePhotomaton W={W} H={H} p={palette} user={user} label={label} namesText={namesText} dateText={dateText} photoUrls={urls} elementStyles={elementStyles}/>;
  }
  ...
  if (tplCfg?.paperImage) {
    ...
    if (tplCfg.layoutVariant === "photo")   return <TemplateLettrPhoto W={W} H={H} paperImage={tplCfg.paperImage} p={palette} user={user} label={label} namesText={namesText} dateText={dateText} photoUrl={photoUrl} elementStyles={elementStyles}/>;
    return <TemplateLettre W={W} H={H} paperImage={tplCfg.paperImage} p={palette} user={user} fontPreset={fontPreset} label={label} namesText={namesText} dateText={dateText} locationText={locationText} footer={footer} elementStyles={elementStyles} paperFit={tplCfg.paperFit}/>;
  }
  return null;
}

export function CardFoldModal({ tpl, paletteId, user, isStd, fontPreset, label, namesText, dateText, locationText, footer, photoUrl, photoUrls, elementStyles, customPaperBg, onClose, inline }: {
  tpl: TemplateConfig; paletteId: string; user: UserData; isStd: boolean;
  fontPreset?: string; label?: string; namesText?: string; dateText?: string; locationText?: string; footer?: string; photoUrl?: string; photoUrls?: string[];
  elementStyles?: ElStyles; customPaperBg?: string;
  ...
  return (
    <div className={inline ? "w-full h-full flex flex-col items-center justify-center gap-6" : "fixed inset-0 z-50 flex flex-col items-center justify-center gap-8"}
      ...
      <div style={{ position: "relative", perspective: "1800px", perspectiveOrigin: "50% 50%", overflow: "visible" }}>
        ...
        {phase === 3 && !inline && (
          <div style={{ position: "absolute", left: Math.round(W * 1.5) + 32, top: "50%", transform: "translateY(-50%)", display: "flex", flexDirection: "column", gap: "10px" }}>
            <button onClick={handleReplay} className="flex items-center gap-2 px-5 py-2.5 rounded-full text-sm font-bold" style={{ backgroundColor: "#6D1D3E", color: "white", fontFamily: "var(--font-display)", whiteSpace: "nowrap" }}>
      ...
      {inline && phase === 3 && (
        <button onClick={handleReplay} className="absolute flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-bold" style={{ left: 28, top: "50%", transform: "translateY(-50%)", backgroundColor: "#6D1D3E", color: "white", fontFamily: "var(--font-display)", boxShadow: "0 4px 16px rgba(109,29,62,0.3)" }}>
...
export function CardFlipScene({ tpl, paletteId, user, isStd, fontPreset, label, namesText, dateText, locationText, footer, photoUrl, photoUrls, elementStyles, customPaperBg, onAnimationDone }: {
  tpl: TemplateConfig; paletteId: string; user: UserData; isStd: boolean;
  ...
  return (
    <div className="w-full h-full flex flex-col items-center justify-center" style={{ position: "relative", perspective: "1400px" }}>
      ...
      {done && (
        <button onClick={() => { setKey(k => k + 1); onAnimationDone && (setDone(false)); }} className="absolute flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-bold" style={{ left: 28, top: "50%", transform: "translateY(-50%)", backgroundColor: "#6D1D3E", color: "white", fontFamily: "var(--font-display)", boxShadow: "0 4px 16px rgba(109,29,62,0.3)" }}>
          <RotateCcw size={14}/> Revoir
        </button>
      )}
    </div>
